...
Future<ApiResponse> getPosts() async {
  ...
  try {
    ...
    final response = await http.get(Uri.parse(postsMazadURL), headers: {
    ...
    switch (response.statusCode) {
      case 200:
        apiResponse.data = jsonDecode(response.body)['postsmaz']
            .map((p) => Posmaz.fromJson(p))
            .toList();
...
Future<ApiResponse> createPost(String body, String? image) async {
  ...
  try {
    String token = await getToken();
    final response = await http.post(Uri.parse(postsMazadURL),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        },
...
Future<ApiResponse> editPost(int postId, String body) async {
  ...
  try {
    String token = await getToken();
    final response = await http.put(Uri.parse('$postsMazadURL/$postId'), headers: {
      'Accept': 'application/json',
      'Authorization': 'Bearer $token'
    }, body: {
      'body': body
...
Future<ApiResponse> likeUnlikePost(int postId) async {
  ...
  try {
    String token = await getToken();
    final response = await http.post(Uri.parse('$postsMazadURL/$postId/likesMazad'),
        headers: {
          'Accept': 'application/json',
          'Authorization': 'Bearer $token'
        });
